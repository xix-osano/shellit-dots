...
install-python-packages(){
  UV_NO_MODIFY_PATH=1
  SHELLIT_VIRTUAL_ENV=$XDG_STATE_HOME/quickshell/.venv
  x mkdir -p $(eval echo $SHELLIT_VIRTUAL_ENV)
  # we need python 3.12 https://github.com/python-pillow/Pillow/issues/8089
  x uv venv --prompt .venv $(eval echo $SHELLIT_VIRTUAL_ENV) -p 3.12
  x source $(eval echo $SHELLIT_VIRTUAL_ENV)/bin/activate
