...
This is important because there has been so many complaints about the failure installing/updating python packages via system package manager.
...
For summary:
- They will be installed to the virtual environment `$SHELLIT_VIRTUAL_ENV`.
- The default value of `$SHELLIT_VIRTUAL_ENV` is `$XDG_STATE_HOME/quickshell/.venv`.
  - The default value of `$XDG_STATE_HOME` is `$HOME/.local/state`.
- Currently we use `env = SHELLIT_VIRTUAL_ENV, ~/.local/state/quickshell/.venv` in `~/.config/hypr/hyprland/env.conf` to set this environment variable.[^2]
...
```bash
source $(eval echo $SHELLIT_VIRTUAL_ENV)/bin/activate
```
...
Example: In `~/‎.config/quickshell/shellit/screenshot.qml`:
```qml
Process {
...
```python
#!/usr/bin/env -S\_/bin/sh\_-c\_"source\_\$(eval\_echo\_\$SHELLIT_VIRTUAL_ENV)/bin/activate&&exec\_python\_-E\_"\$0"\_"\$@""
```
And that's it!
...
In this example, in `~/‎.config/quickshell/shellit/screenshot.qml` we should modify `find_regions.py` to the wrapper script `find-regions-venv.sh`:
```qml
Process {
id: imageDetectionProcess
