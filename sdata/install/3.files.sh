...
case $SKIP_MISCCONF in
  true) sleep 0;;
  *)
    for i in $(find dots/.config/ -mindepth 1 -maxdepth 1 ! -name 'quickshell' ! -name 'fish' ! -name 'fontconfig' -exec basename {} \;); do
#      i="dots/.config/$i"
...
case $SKIP_QUICKSHELL in
  true) sleep 0;;
  *)
     # Should overwriting the whole directory not only ~/.config/quickshell/shellit/ cuz https://github.com/end-4/dots-hyprland/issues/2294#issuecomment-3448671064
...
case $SKIP_HYPRLAND in
  ...
  *)
    ...
    if [ -d "$XDG_CONFIG_HOME/hypr/custom" ];then shellit_firstrun=false;else shellit_firstrun=true;fi
    t="$XDG_CONFIG_HOME/hypr/hyprland.conf"
    if [ -f $t ];then
      echo -e "${STY_BLUE}[$0]: \"$t\" already exists.${STY_RST}"
      if $shellit_firstrun;then
...
v cp -f "dots/.local/share/icons/shellit.svg" "${XDG_DATA_HOME}"/icons/shellit.svg
...
if [[ -z "${SHELLIT_VIRTUAL_ENV}" ]]; then
  printf "\n${STY_RED}[$0]: \!! Important \!! : Please ensure environment variable ${STY_RST} \$SHELLIT_VIRTUAL_ENV ${STY_RED} is set to proper value (by default \"~/.local/state/quickshell/.venv\"), or Quickshell config will not work. We have already provided this configuration in ~/.config/hypr/hyprland/env.conf, but you need to ensure it is included in hyprland.conf, and also a restart is needed for applying it.${STY_RST}\n"
fi
