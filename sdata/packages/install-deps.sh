...
handle-deprecated-dependencies(){
  ...
  for i in shellit-{microtex,pymyc-aur} {quickshell,hyprutils,hyprpicker,hyprlang,hypridle,hyprland-qt-support,hyprland-qtutils,hyprlock,xdg-desktop-portal-hyprland,hyprcursor,hyprwayland-scanner,hyprland}-git;do try sudo pacman --noconfirm -Rdd $i;done
# Convert old dependencies to non explicit dependencies so that they can be orphaned if not in meta packages
  remove_bashcomments_emptylines ./sdata/packages/previous_dependencies.conf ./cache/old_deps_stripped.conf
...
metapkgs=(./sdata/packages/shellit-{audio,backlight,basic,fonts-themes,kde,portal,python,screencapture,toolkit,widgets})
metapkgs+=(./sdata/packages/shellit-hyprland)
metapkgs+=(./sdata/packages/shellit-microtex-git)
metapkgs+=(./sdata/packages/shellit-quickshell-git)
# metapkgs+=(./sdata/packages/shellit-oneui4-icons-git)
[[ -f /usr/share/icons/Bibata-Modern-Classic/index.theme ]] || \
  metapkgs+=(./sdata/packages/shellit-bibata-modern-classic-bin)
