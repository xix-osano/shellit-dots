...
Item {
    ...
    readonly property bool isRightEdge: SettingsData.ShellitBarPosition === SettingsData.Position.Right
    ...
    function loadArtwork(url) {
        ...
        if (url.startsWith("http://") || url.startsWith("https://")) {
            const filename = "/tmp/.Shellitshell/trackart_" + Date.now() + ".jpg"
            activeTrackArtFile = filename

            cleanupProcess.command = ["sh", "-c", "mkdir -p /tmp/.Shellitshell && find /tmp/.Shellitshell -name 'trackart_*' ! -name '" + filename.split('/').pop() + "' -delete"]
