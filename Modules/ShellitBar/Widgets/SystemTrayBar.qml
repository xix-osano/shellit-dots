...
Item {
    ...
    Rectangle {
        ...
        radius: SettingsData.ShellitBarNoBackground ? 0 : Theme.cornerRadius
        color: {
            if (visibleTrayItems.length === 0) {
                return "transparent";
            }

            if (SettingsData.ShellitBarNoBackground) {
                return "transparent";
            }
        ...
        Rectangle {
            ...
            PanelWindow {
                ...
                function updatePosition() {
                    ...
                    const widgetThickness = Math.max(20, 26 + SettingsData.ShellitBarInnerPadding * 0.6)
                    const effectiveBarThickness = Math.max(widgetThickness + SettingsData.ShellitBarInnerPadding + 4, Theme.barHeight - 4 - (8 - SettingsData.ShellitBarInnerPadding))

                    if (menuRoot.isVertical) {
                        const edge = menuRoot.axis?.edge
                        let targetX
                        if (edge === "left") {
                            targetX = effectiveBarThickness + SettingsData.ShellitBarSpacing + Theme.popupDistance
                        } else {
                            const popupX = effectiveBarThickness + SettingsData.ShellitBarSpacing + Theme.popupDistance
                        ...
                        if (menuRoot.isAtBottom) {
                            const popupY = effectiveBarThickness + SettingsData.ShellitBarSpacing + SettingsData.ShellitBarBottomGap + Theme.popupDistance
                            targetY = screen.height - popupY
                        } else {
                            targetY = effectiveBarThickness + SettingsData.ShellitBarSpacing + SettingsData.ShellitBarBottomGap + Theme.popupDistance
                        }
