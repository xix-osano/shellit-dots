...
PanelWindow {
    ...
    function getTopMargin() {
        ...
        const barPos = SettingsData.ShellitBarPosition
        const isTop = isTopCenter || popupPos === SettingsData.Position.Top || popupPos === SettingsData.Position.Left

        if (!isTop) return 0

        const effectiveBarThickness = Math.max(26 + SettingsData.ShellitBarInnerPadding * 0.6 + SettingsData.ShellitBarInnerPadding + 4, Theme.barHeight - 4 - (8 - SettingsData.ShellitBarInnerPadding))
        const exclusiveZone = effectiveBarThickness + SettingsData.ShellitBarSpacing + SettingsData.ShellitBarBottomGap
