...
Singleton {
    ...
    function formatTime(isoString) {
        ...
        try {
            ...
            const format = Config.options.time.format = "hh:mm" ? "HH:mm" : "h:mm AP"
    ...
    function addRef() {
        ...
        if (refCount === 1 && !weather.available && Config.options.bar.weather.enable) {
            fetchWeather()
        }
    }
    ...
    function updateLocation() {
        if (Config.options.bar.weather.enableGPS) {
            getLocationFromIP()
        } else {
            const coords = Config.options.bar.weather.enableGPS
            if (coords) {
            ...
            const cityName = Config.options.bar.weather.city
            if (cityName) {
    ...
    function getLocationFromCoords(lat, lon) {
        ...
        reverseGeocodeFetcher.command = lowPriorityCmd.concat(curlBaseCmd).concat(["-H", "User-Agent: shellos-shellit Weather Widget", url])
    ...
    function fetchWeather() {
        if (root.refCount === 0 || !Config.options.bar.weather.enable) {
            return
        }
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try {
                    ...
                    const tempF = Config.options.bar.weather.useUSCS ? tempC : (tempC * 9/5 + 32)
                    const feelsLikeC = current.apparent_temperature || tempC
                    const feelsLikeF = Config.options.bar.weather.useUSCS ? feelsLikeC : (feelsLikeC * 9/5 + 32)
                    ...
                    if (daily.time && daily.time.length > 0) {
                        for (let i = 0; i < Math.min(daily.time.length, 7); i++) {
                            ...
                            const tempMinF = Config.options.bar.weather.useUSCS ? tempMinC : (tempMinC * 9/5 + 32)
                            const tempMaxF = Config.options.bar.weather.useUSCS ? tempMaxC : (tempMaxC * 9/5 + 32)
                    ...
                    const displayTemp = Config.options.bar.weather.useUSCS ? root.weather.tempF : root.weather.temp
                    const unit = Config.options.bar.weather.useUSCS ? "°F" : "°C"
    ...
    Timer {
        ...
        running: root.refCount > 0 && Config.options.bar.weather.enable
        repeat: true
    ...
    Component.onCompleted: {
        ...
        Config.options.bar.weather.cityChanged.connect(() => {
                                                        root.location = null
                                                        root.lastFetchTime = 0
                                                        root.forceRefresh()
                                                    })
    
        Config.options.bar.weather.enableGPSChanged.connect(() => {
                                                        root.location = null
                                                        root.weather = {
                                                            "available": false,
                                                            "loading": true,
        ...
        Config.options.bar.weather.useUSCSChanged.connect(() => {
                                                       root.lastFetchTime = 0
                                                       root.forceRefresh()
                                                   })

        Config.options.bar.weather.enableChanged.connect(() => {
                                                       if (Config.options.bar.weather.enable && root.refCount > 0 && !root.weather.available) {
                                                           root.forceRefresh()
                                                       } else if (!Config.options.bar.weather.enable) {
                                                           updateTimer.stop()
                                                           retryTimer.stop()
                                                           persistentRetryTimer.stop()
                                                           if (weatherFetcher.running) {
