...
Singleton {
    ...
    property string keyringLabel: Translation.tr("%1 Safe Storage").arg("shellos-shellit")
    ...
    Process {
        ...
        command: [ // We need to use echo for a newline so splitparser does parse
            "bash", "-c", `echo $(secret-tool lookup 'application' 'shellos-shellit')`,
        ]
        stdout: SplitParser {
            onRead: data => {
